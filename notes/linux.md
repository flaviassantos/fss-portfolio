### This note is for the [DevOps Bootcamp] for "Operating Systems & Linux Basics" Module ###



![Linux Icon for Linux Module](img/linux.png "OS & Linux Basics Module")

**Linux File Structure**

![Linux File System](img/unix-filesystem-hierarchy.png )
...
- `tree -L 1` = show one tree level below the current directory
- `history 20` = Show list of last 20 commands used in the terminal
- `CTRL + r` = Search history
- `CTRL + c` = Stop current command
- `CTRL + SHIFT + v` = Paste copied text into terminal
- `ls -a` = See hidden files too
- `cat [dir/filename]` = Display the file content
 
Display OS Information
- `uname -a` = Show system and kernel
- `cat /etc/os-release` =  Show OS information
- `lscpu` = Display hardware information, e.g. how many CPU you have etc.
- `sysctl -a | grep machdep.cpu` = for macOS
...
- /etc/passwd
  ...
  ![Example](img/example_password_line.png)
  - Everyone can read it, but only root user can change the file


- /etc/shadow

  - system file in Linux that stores encrypted user passwords and is accessible only to the root user.
